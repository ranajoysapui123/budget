...
export class StudentService {
  ...
  async updateFeePayment(id: string, updates: Partial<FeePayment>): Promise<void> {
    try {
      ...
      if (error.message?.includes('UNIQUE constraint failed')) {
        throw new Error('Fee record update failed: Duplicate record exists');
      }
      throw error;
    }
  }

  async getFeeRecord(studentId: string, month: string, year: number): Promise<FeePayment | undefined> {
    const stmt = this.db.prepare(`
      SELECT 
        id, student_id as studentId, month, year,
    ...
    return stmt.get(studentId, month, year) as FeePayment | undefined;
  }
  ...
  async getStudentsWithSubjectsAndPayments(month: string, year: number) {
    const stmt = this.db.prepare(`
      SELECT 
        s.id as student_id,
        s.name as student_name,
        subj.name as subject_name,
        COALESCE(ss.custom_fee, subj.base_fee) as fee,
        CASE 
          WHEN fp.status = 'paid' THEN 1
          ELSE 0
        END as paid
      FROM students s
      JOIN student_subjects ss ON s.id = ss.student_id
      JOIN subjects subj ON ss.subject_id = subj.id
      LEFT JOIN fee_payments fp ON 
        s.id = fp.student_id AND 
        fp.month = ? AND 
        fp.year = ?
  ...
  async getStudentsWithSubjectsAndFees(month: string, year: number) {
    const stmt = this.db.prepare(`
      SELECT 
        s.id as student_id,
        s.name as student_name,
  ...
  // Get students with their detailed subjects
  async getStudentsWithDetailedSubjects(): Promise<(Student & { subjects: Array<{ name: string; fee: number }> })[]> {
    const students = await this.getAllStudents();
